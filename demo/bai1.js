...
// <id> - <title> - <rating>
const result2 = courses.filter(course => course.rating < 4).map(course => course.id + " - " + course.title + " - " +course.rating);
// const result3 = courses.filter(course => course.rating < 4).map(course => `${course.id} - ${course.title} - ${course.rating}`);
console.log(result2);
